...
namespace DTDanmakuLib
{
	...
	public class SniperEnemyGenerator
	{
			...
			GuidGenerator guidGenerator)
		{
			...
			return ObjectAction.Union(
				...
				ObjectAction.SetNumericVariable(variableName, MathExpression.Add(MathExpression.Variable(variableName), MathExpression.Multiply(MathExpression.ElapsedMillisecondsPerIteration(), MathExpression.Constant(270)))),
				...
				ObjectAction.SetFacingDirection(MathExpression.Variable(variableName)));
		}

		private static ObjectAction GetBulletMovementAction(
			GuidGenerator guidGenerator)
		{
			long bulletSpeedInPixelsPerSecond = 100;
			...
			GuidGenerator guidGenerator)
		{
			...
			spriteNameToImageDictionary.Add(enemyBulletSpriteName, DTDanmakuImage.SniperEnemyBullet);
