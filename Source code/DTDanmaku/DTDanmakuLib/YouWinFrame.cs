...
namespace DTDanmakuLib
{
	...
	public class YouWinFrame : IFrame<IDTDanmakuAssets>
	{
		private int fps;
		...
		public YouWinFrame(int fps, IDTDeterministicRandom rng, GuidGenerator guidGenerator, int soundVolume, bool debugMode)
		{
			this.fps = fps;
			this.rng = rng;
		...
		public void Render(IDisplay<IDTDanmakuAssets> display)
		{
			...
			int width = (int)display.GetAssets().GetWidth(DTDanmakuImage.YouWin);
			int height = (int)display.GetAssets().GetHeight(DTDanmakuImage.YouWin);
			display.GetAssets().DrawImage(DTDanmakuImage.YouWin, 1000 / 2 - width / 2, 700 / 2 - height / 2);
		}
	}
}
